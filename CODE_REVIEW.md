...
> ## 📌 这是历史文档
>
> 本文写于 **2026-08-09**，针对的是提交 `e001d49`（`index.html` 1004 行，音源还是 `<audio>` 元素）。
> 后续 `538e03c` 已把播放引擎重写为 **`AudioBuffer` + `AudioBufferSourceNode`**，
> 本文描述的 R1–R6 / Y1–Y14 **多数已修复或已被重写覆盖**，行号也早已失效。
>
> 保留它是因为记录了一批真实的设计缺陷与当时的推理过程，仍有参考价值。
> **看当前代码请直接读 `index.html`**，不要拿本文的行号去对照。
